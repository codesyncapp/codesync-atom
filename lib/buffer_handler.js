...
import { RESTART_DAEMON_AFTER, DIFFS_REPO, DIFF_FILES_PER_ITERATION, USER_PATH, NOTIFICATION,
						...
						if (resp.type === 'auth') {
							if (resp.status !== 200) {
								...
								notifications.forEach((notification) => {
									alreadyShowing = (!notification.dismissed && notification.type === 'warning' && notification.message === NOTIFICATION.AUTHENTICATION_FAILED) || alreadyShowing;
								});
								putLogEvent(NOTIFICATION.AUTH_FAILED_FOR_DIFF);
								if (!alreadyShowing) {
									askAndTriggerSignUp();
								}
								return;
