...
import { isValidDiff, handleNewFileUpload, handleFilesRename,
	cleanUpDeleteDiff, getDIffForDeletedFile } from "./utils/buffer_utils";

import {
	RESTART_DAEMON_AFTER, DIFFS_REPO, DIFF_FILES_PER_ITERATION, USER_PATH,
	CONFIG_PATH, WEBSOCKET_ENDPOINT, ORIGINALS_REPO, DEFAULT_BRANCH, STATUS_BAR_MSGS
} from "./constants";
import { syncRepo } from './init_handler';
import { initUtils } from './utils/init_utils';

const recallDaemon = (statusBarItem) => {
...
export const detectBranchChange = async (viaDaemon=true) => {
	...
	for (const repoPath of Object.keys(configJSON.repos)) {
		...
		const accessToken = users[userEmail].access_token;
        if (!accessToken) {
			putLogEvent(`Access token not found for repo: ${repoPath}. email: ${userEmail}`, userEmail);
			continue;
		}
...
export async function handleBuffer(statusBarItem) {
    /***
	...
	***/
	...
	try {
		...
		if (!diffFiles.length) {
			...
			const msg = repoPath ? STATUS_BAR_MSGS.DEFAULT : STATUS_BAR_MSGS.NO_REPO_OPEN;
