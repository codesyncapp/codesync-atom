...
import {formatDatetime, getBranch, getSkipRepos, getSyncIgnoreItems, isEmpty, readYML} from "../utils/common";
...
const populateBufferForMissedEvents = async (readyRepos, viaDaemon=false) => {
    for (const repoPath of Object.keys(readyRepos)) {
        ...
        global.lastSyncedAt[repoPath] = obj.repoModifiedAt;
    }
};
...
class PopulateBuffer {
  ...
  */
    ...
    constructor(repoPath, branch, viaDaemon) {
        ...
        this.itemPaths = this.initUtilsObj.getSyncablePaths({},true);
        ...
        for (const itemPath of this.itemPaths) {
            ...
            if (itemPath.is_binary) {
                if (!fileInConfig) {
                    ...
                    handler.handleCreate(itemPath.file_path);
                }
                continue;
            }
    ...
    * */
    ...
    for (const repoPath of Object.keys(configJSON.repos)) {
        ...
        if (branch in configRepo.branches) {
            ...
            if (shouldSyncBranch) {
                const itemPaths = initUtilsObj.getSyncablePaths({}, true);
