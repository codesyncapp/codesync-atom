...
class PopulateBuffer {
    ...
    async populateBufferForRepo() {
        ...
        for (const itemPath of this.itemPaths) {
            ...
            if (isNewFile) {
                diff = "";
                initUtilsObj.copyFilesTo(this.repoPath, [itemPath.file_path], originalsRepoBranchPath);
            }
            // Sync file in shadow repo with latest content
            initUtilsObj.copyFilesTo(this.repoPath, [itemPath.file_path], shadowRepoBranchPath);
    ...
    getDiffForDeletedFiles() {
        ...
        */
        ...
        Object.keys(this.configFiles).forEach(relPath => {
            ...
            initUtilsObj.copyFilesTo(this.repoPath, [shadowFilePath], cacheRepoPath);
        });
        return diffs;
    }
}
...
export const detectBranchChange = async () => {
    ...
    * */
    ...
    for (const repoPath of Object.keys(configJSON.repos)) {
        ...
        const shadowRepo = path.join(this.settings.SHADOW_REPO, repoPath);
