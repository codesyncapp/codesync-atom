...
export function handleFileCreated(event) {
    ...
    const branch = getBranchName({altPath: repoPath});
    const destOriginals = `${settings.ORIGINALS_REPO}/${repoPath.slice(1)}/${branch}/${relPath}`;
...
function handleRename(repoPath, branch, oldAbsPath, newAbsPath, isFile) {
    ...
    const newRelPath = newAbsPath.split(`${repoPath}/`)[1];
    const oldShadowPath = `${settings.SHADOW_REPO}/${repoPath.slice(1)}/${branch}/${oldRelPath}`;
    const newShadowPath = `${settings.SHADOW_REPO}/${repoPath.slice(1)}/${branch}/${newRelPath}`;
