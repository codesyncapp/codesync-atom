...
const postClickVisibility = async (repoPath, repoName, branch, user, accessToken, isPublic,
    ...
    const itemPaths = initUtilsObj.getSyncablePaths(repoPath, user.plan, isSyncingBranch);
    ...
    initUtilsObj.copyFilesTo(repoPath, filePaths, originalsRepoBranchPath);

    const shadowRepoBranchPath = path.join(settings.SHADOW_REPO, path.join(repoPath, branch));
    // copy files to .shadow repo
    initUtilsObj.copyFilesTo(repoPath, filePaths, shadowRepoBranchPath);

    // Upload repo/branch
    await initUtilsObj.uploadRepo(repoPath, branch, accessToken, itemPaths, isPublic, isRepoSynced, viaDaemon, user.email);
}
