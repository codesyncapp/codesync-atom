...
export class initUtils {
    ...
    copyFilesTo(filePaths, destination) {
        filePaths.forEach((filePath) => {
            // Need formatted repo path for .shadow, .originals & .deleted directories
    ...
    async uploadRepoToS3(branch, token, uploadResponse, userEmail,
        ...
        const tasks = [];
        const originalsRepoBranchPath = path.join(this.settings.ORIGINALS_REPO, path.join(this.repoPath, branch));
