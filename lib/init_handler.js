...
export const syncRepo = async (repoPath, accessToken, email, viaDaemon=false, isSyncingBranch=false) => {
    ...
    atom.notifications.addInfo(NOTIFICATION.UPDATE_SYNCIGNORE, 
        {
            buttons: [
                {
                    text: NOTIFICATION.CONTINUE,
                    onDidClick: async () => await postSyncignoreUpdated(repoPath, repoName, branch, user, accessToken, viaDaemon, isRepoSynced)
                },
                {
                    text: NOTIFICATION.CANCEL,
                    onDidClick: () => atom.notifications.addWarning(NOTIFICATION.INIT_CANCELLED)
                }
        ]
    });
}
            ...
            buttons: [
                {
                    ...
                    onDidClick: () => postClickVisibility(repoPath, repoName, branch, user, accessToken, true, viaDaemon, isRepoSynced)
                },
                {
                    text: NOTIFICATION.NO,
                    onDidClick: () => postClickVisibility(repoPath, repoName, branch, user, accessToken, false, viaDaemon, isRepoSynced)
                },
...
const postClickVisibility = async (repoPath, repoName, branch, user, accessToken, isPublic, viaDaemon, isRepoSynced) => {
