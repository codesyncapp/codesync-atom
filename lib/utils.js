...
export function handleFileRenamed(event) {
    ...
    if (fs.lstatSync(newAbsPath).isDirectory()) {
        fs.readdirSync(newAbsPath).forEach(file => {
            ...
            handleRename(repoName, repoPath, oldFilePath, newFilePath);
        });
        return;
    }
    if (fs.lstatSync(newAbsPath).isFile()) {
        handleRename(repoName, repoPath, oldAbsPath, newAbsPath);
    }
}
...
function handleRename(repoName, repoPath, oldAbsPath, newAbsPath) {
