...
export const initExpressServer = () => {
    ...
    expressApp.listen(global.port, () => {
        console.log(`server started at ${createRedirectUri()}`);
    });
};
...
export async function handleRedirect(req) {
...
export function createRedirectUri() {
    return `${Auth0URLs.REDIRECT_URI}:${global.port}`;
}
