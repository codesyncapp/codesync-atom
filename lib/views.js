...
import { getProjectPath, readYML } from "./utils/common";
...
export class panelView {
  ...
  getElement(obj) {
    ...
    if ([NOTIFICATION.LOGGED_OUT_SUCCESSFULLY].includes(obj.message)) {
      enableLoginView();
    }

    else if ([NOTIFICATION.CONNECT_AFTER_JOIN, NOTIFICATION.CONNECT_REPO, NOTIFICATION.REPO_UNSYNCED].includes(obj.message)) {
      enableConnectRepoView();
    }

    else if ([NOTIFICATION.REPO_SYNCED].includes(obj.message) || obj.message.endsWith(NOTIFICATION.REPO_IN_SYNC)) {
...
const enableLoginView = () => {
  ...
  panels.forEach((panel) => {
    if (panel.item.className === "codesync-view") {
      ...
      // Show new panel
      const loginViewInstance = new LoginView();
      const view = atom.views.getView(loginViewInstance);
